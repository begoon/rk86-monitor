...
- `monitor.json` — structured listing emitted by `asm8080 -l`; consumed by the web viewer. Regenerated by `make listing` or `make docs`
- `ui/` — Svelte 5 + Vite source for the interactive web viewer
- `docs/index.html` — single-file build output (GH Pages-ready; commit after `make docs`)
...
make listing  # regenerate monitor.json/.lst/.sym/.map
make test     # compare output with original/mon32.bin
make docs     # regenerate listing + build single-file ui/ → docs/index.html
make clean
```

Assembler: `asm8080` from npm (`bunx --bun asm8080@latest`). Requires Node.js; Bun is used for the UI build (`bun install` + `vite build`).
